...
get_github_release() {
    local repo="$1"
    local pattern="$2"
    curl -s -H "Authorization: token ${GH_TOKEN}" "${GH_API}/${repo}/releases/latest" | \
...
get_github_release_any() {
    local repo="$1"
    local pattern="$2"
    curl -s -H "Authorization: token ${GH_TOKEN}" "${GH_API}/${repo}/releases" | \
    grep "browser_download_url" | \
    grep -oE "https.*${pattern}" | \
    head -n 1
}
...
get_github_release_tags() {
    ...
    local tags="$2"
    local pattern="$3"
    curl -s -H "Authorization: token ${GH_TOKEN}" "${GH_API}/${repo}/releases/tags/${tags}" | \
    grep "browser_download_url" | \
    grep -oE "https.*${pattern}" | \
    head -n 1
}
...
determine_core_files() {
    ...
    passwall_core_url=$(get_github_release "xiaorouji/openwrt-passwall" "$passwall_core_zip_pattern")

    # Nikki core (.tar.gz dari release)
    nikki_core_pattern="nikki_${ARCH_3}-openwrt-${VEROP}.*\.tar.gz"
    nikki_core_url=$(get_github_release "nikkinikki-org/OpenWrt-nikki" "$nikki_core_pattern")
}
